...
VisualWP Cloudflare Turnstile relies on [Cloudflare Turnstile](https://www.cloudflare.com/products/turnstile/), a 3rd party API that connects to Cloudflare servers to generate the anti-spam field that appears on the WordPress login form, register form, comment form and any Ninja Form that the field is manually added to. The anti-spam check will not work if website traffic to Cloudflare servers is blocked or impeded. Cloudflare has to look at some session data (like headers, user agent, and browser characteristics) to validate users without challenging them. Learn more by visiting Cloudflare  [Terms of Service](https://www.cloudflare.com/website-terms/) and [Privacy Policy](https://www.cloudflare.com/privacypolicy/)
